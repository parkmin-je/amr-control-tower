/* ── AMR Control Tower — i18n ───────────────────────────────── */
const TRANSLATIONS = {
  en: {
    'nav.dashboard':       'Dashboard',
    'nav.fleet':           'Fleet',
    'nav.tasks':           'Tasks',
    'nav.admin':           'Admin',
    'page.title':          'Dashboard',
    'label.state':         'State',
    'label.telemetry':     'Telemetry',
    'label.battery':       'Battery',
    'label.session':       'Session',
    'label.robotId':       'Robot ID',
    'label.posX':          'Pos X',
    'label.posY':          'Pos Y',
    'label.yaw':           'Yaw',
    'label.linearVel':     'Linear vel',
    'label.angularVel':    'Angular vel',
    'label.distance':      'Distance (m)',
    'label.uptime':        'Uptime (s)',
    'map.title':           'SLAM Map',
    'map.lidar':           'LiDAR',
    'map.clearPath':       'Clear path',
    'map.hint':            'Click map to send nav goal',
    'chart.linearVel':     'Linear Velocity',
    'chart.ms':            'm/s',
    'chart.mission':       'Mission History',
    'ctrl.command':        'Command',
    'ctrl.estop':          'E-Stop',
    'ctrl.resume':         'Resume',
    'ctrl.sendGoal':       'Send goal',
    'ctrl.navRemaining':   'Remaining',
    'ctrl.navSucceeded':   'Arrived ✓',
    'ctrl.navAborted':     'Navigation failed ✗',
    'ctrl.goalFail':       'Goal send failed',
    'ctrl.labelX':         'X (m)',
    'ctrl.labelY':         'Y (m)',
    'ctrl.labelTheta':     'Theta',
    'ctrl.velLimits':      'Velocity Limits',
    'ctrl.linear':         'Linear',
    'ctrl.angular':        'Angular',
    'ctrl.manualDrive':    'Manual Drive',
    'ctrl.kbHint':         'WASD / Arrows / Space=Stop',
    'event.title':         'Event Log',
    'conn.connected':      'Connected',
    'conn.disconnected':   'Disconnected',
    'label.units':         'Units',
    'state.IDLE':          'Idle',
    'state.MOVING':        'Moving',
    'state.EMERGENCY_STOP':'E-Stop',
    'state.CHARGING':      'Charging',
    'state.ERROR':         'Error',
    'state.OFFLINE':       'Offline',
    'fleet.updated':       'Updated',
    'task.newTask':        'NEW TASK',
    'task.robot':          'Robot',
    'task.title':          'Title',
    'task.type':           'Type',
    'task.priority':       'Priority (1=high)',
    'task.description':    'Description',
    'task.create':         'Create Task',
    'task.allTasks':       'ALL TASKS',
    'task.id':             'ID',
    'task.status':         'STATUS',
    'task.createdBy':      'CREATED BY',
    'task.createdAt':      'CREATED AT',
    'task.actions':        'ACTIONS',
    'admin.addUser':       'ADD USER',
    'admin.username':      'Username',
    'admin.password':      'Password',
    'admin.role':          'Role',
    'admin.users':         'USERS',
    'admin.status':        'STATUS',
    'admin.actions':       'ACTIONS',
    'admin.registerRobot': 'REGISTER ROBOT',
    'admin.robotId':       'Robot ID',
    'admin.displayName':   'Display Name',
    'admin.rosbridgeUri':  'RosBridge URI',
    'admin.topicSettings': 'Topic Settings (optional)',
    'admin.robots':        'ROBOTS',
    'admin.assignedRobots':'ASSIGNED ROBOTS',
    'admin.assign':        'Assign',
    'fleet.estopAll':      'Fleet E-Stop',
    'toast.estopOk':       'Emergency stop executed',
    'toast.estopFail':     'E-Stop failed',
    'toast.resumeOk':      'Resumed',
    'toast.resumeFail':    'Resume failed',
    'toast.goalOk':        'Goal sent',
    'toast.goalFail':      'Goal send failed',
    'toast.fleetEstopOk':  'Fleet E-Stop executed',
    'toast.fleetEstopFail':'Fleet E-Stop failed',
    'toast.permDenied':    'Permission denied for this robot',
  },
  ko: {
    'nav.dashboard':       '대시보드',
    'nav.fleet':           '플리트',
    'nav.tasks':           '태스크',
    'nav.admin':           '관리자',
    'page.title':          '대시보드',
    'label.state':         '상태',
    'label.telemetry':     '텔레메트리',
    'label.battery':       '배터리',
    'label.session':       '세션',
    'label.robotId':       '로봇 ID',
    'label.posX':          '위치 X',
    'label.posY':          '위치 Y',
    'label.yaw':           '방향각',
    'label.linearVel':     '선속도',
    'label.angularVel':    '각속도',
    'label.distance':      '이동거리 (m)',
    'label.uptime':        '가동시간 (s)',
    'map.title':           'SLAM 맵',
    'map.lidar':           '라이다',
    'map.clearPath':       '경로 초기화',
    'map.hint':            '맵 클릭 → 목표 전송',
    'chart.linearVel':     '선속도',
    'chart.ms':            'm/s',
    'chart.mission':       '미션 히스토리',
    'ctrl.command':        '명령',
    'ctrl.estop':          '긴급 정지',
    'ctrl.resume':         '재개',
    'ctrl.sendGoal':       '목표 전송',
    'ctrl.navRemaining':   '남은 거리',
    'ctrl.navSucceeded':   '도착 완료 ✓',
    'ctrl.navAborted':     '내비게이션 실패 ✗',
    'ctrl.goalFail':       '목표 전송 실패',
    'ctrl.labelX':         'X (m)',
    'ctrl.labelY':         'Y (m)',
    'ctrl.labelTheta':     '방향 (rad)',
    'ctrl.velLimits':      '속도 제한',
    'ctrl.linear':         '선속도',
    'ctrl.angular':        '각속도',
    'ctrl.manualDrive':    '수동 조종',
    'ctrl.kbHint':         'WASD / 방향키 / Space=정지',
    'event.title':         '이벤트 로그',
    'conn.connected':      '연결됨',
    'conn.disconnected':   '연결 끊김',
    'label.units':         '대수',
    'state.IDLE':          '대기',
    'state.MOVING':        '이동 중',
    'state.EMERGENCY_STOP':'긴급 정지',
    'state.CHARGING':      '충전 중',
    'state.ERROR':         '오류',
    'state.OFFLINE':       '오프라인',
    'fleet.updated':       '업데이트',
    'task.newTask':        '새 태스크',
    'task.robot':          '로봇',
    'task.title':          '제목',
    'task.type':           '유형',
    'task.priority':       '우선순위 (1=높음)',
    'task.description':    '설명',
    'task.create':         '태스크 생성',
    'task.allTasks':       '전체 태스크',
    'task.id':             'ID',
    'task.status':         '상태',
    'task.createdBy':      '생성자',
    'task.createdAt':      '생성 시각',
    'task.actions':        '액션',
    'admin.addUser':       '사용자 추가',
    'admin.username':      '사용자명',
    'admin.password':      '비밀번호',
    'admin.role':          '역할',
    'admin.users':         '사용자',
    'admin.status':        '상태',
    'admin.actions':       '액션',
    'admin.registerRobot': '로봇 등록',
    'admin.robotId':       '로봇 ID',
    'admin.displayName':   '표시명',
    'admin.rosbridgeUri':  'RosBridge URI',
    'admin.topicSettings': '토픽 설정 (선택)',
    'admin.robots':        '로봇 목록',
    'admin.assignedRobots':'담당 로봇',
    'admin.assign':        '배정',
    'fleet.estopAll':      'Fleet 긴급 정지',
    'toast.estopOk':       '긴급 정지 실행',
    'toast.estopFail':     'E-Stop 실패',
    'toast.resumeOk':      '정지 해제',
    'toast.resumeFail':    '해제 실패',
    'toast.goalOk':        '목표 전송 완료',
    'toast.goalFail':      '목표 전송 실패',
    'toast.fleetEstopOk':  'Fleet 긴급 정지 실행',
    'toast.fleetEstopFail':'Fleet 긴급 정지 실패',
    'toast.permDenied':    '이 로봇에 대한 권한이 없습니다',
  },
  ja: {
    'nav.dashboard':       'ダッシュボード',
    'nav.fleet':           'フリート',
    'nav.tasks':           'タスク',
    'nav.admin':           '管理',
    'page.title':          'ダッシュボード',
    'label.state':         '状態',
    'label.telemetry':     'テレメトリ',
    'label.battery':       'バッテリー',
    'label.session':       'セッション',
    'label.robotId':       'ロボットID',
    'label.posX':          '位置 X',
    'label.posY':          '位置 Y',
    'label.yaw':           'ヨー角',
    'label.linearVel':     '線速度',
    'label.angularVel':    '角速度',
    'label.distance':      '走行距離 (m)',
    'label.uptime':        '稼働時間 (s)',
    'map.title':           'SLAMマップ',
    'map.lidar':           'LiDAR',
    'map.clearPath':       '経路クリア',
    'map.hint':            'マップクリックで目標送信',
    'chart.linearVel':     '線速度',
    'chart.ms':            'm/s',
    'chart.mission':       'ミッション履歴',
    'ctrl.command':        'コマンド',
    'ctrl.estop':          '緊急停止',
    'ctrl.resume':         '再開',
    'ctrl.sendGoal':       '目標送信',
    'ctrl.navRemaining':   '残距離',
    'ctrl.navSucceeded':   '到達完了 ✓',
    'ctrl.navAborted':     'ナビ失敗 ✗',
    'ctrl.goalFail':       '目標送信失敗',
    'ctrl.labelX':         'X (m)',
    'ctrl.labelY':         'Y (m)',
    'ctrl.labelTheta':     '向き (rad)',
    'ctrl.velLimits':      '速度制限',
    'ctrl.linear':         '線速度',
    'ctrl.angular':        '角速度',
    'ctrl.manualDrive':    '手動操縦',
    'ctrl.kbHint':         'WASD / 矢印キー / Space=停止',
    'event.title':         'イベントログ',
    'conn.connected':      '接続済み',
    'conn.disconnected':   '切断',
    'label.units':         '台数',
    'state.IDLE':          'アイドル',
    'state.MOVING':        '移動中',
    'state.EMERGENCY_STOP':'緊急停止',
    'state.CHARGING':      '充電中',
    'state.ERROR':         'エラー',
    'state.OFFLINE':       'オフライン',
    'fleet.updated':       '更新時刻',
    'task.newTask':        '新規タスク',
    'task.robot':          'ロボット',
    'task.title':          'タイトル',
    'task.type':           'タイプ',
    'task.priority':       '優先度 (1=高)',
    'task.description':    '説明',
    'task.create':         'タスク作成',
    'task.allTasks':       '全タスク',
    'task.id':             'ID',
    'task.status':         '状態',
    'task.createdBy':      '作成者',
    'task.createdAt':      '作成日時',
    'task.actions':        'アクション',
    'admin.addUser':       'ユーザー追加',
    'admin.username':      'ユーザー名',
    'admin.password':      'パスワード',
    'admin.role':          'ロール',
    'admin.users':         'ユーザー',
    'admin.status':        '状態',
    'admin.actions':       'アクション',
    'admin.registerRobot': 'ロボット登録',
    'admin.robotId':       'ロボットID',
    'admin.displayName':   '表示名',
    'admin.topicSettings': 'トピック設定 (任意)',
    'admin.rosbridgeUri':  'RosBridge URI',
    'admin.robots':        'ロボット一覧',
    'admin.assignedRobots':'担当ロボット',
    'admin.assign':        '割り当て',
    'fleet.estopAll':      'Fleet 緊急停止',
    'toast.estopOk':       '緊急停止実行',
    'toast.estopFail':     '緊急停止失敗',
    'toast.resumeOk':      '再開しました',
    'toast.resumeFail':    '再開失敗',
    'toast.goalOk':        '目標送信完了',
    'toast.goalFail':      '目標送信失敗',
    'toast.fleetEstopOk':  'Fleet緊急停止実行',
    'toast.fleetEstopFail':'Fleet緊急停止失敗',
    'toast.permDenied':    'このロボットへの権限がありません',
  }
};

function getLang() {
  return localStorage.getItem('amr-lang') || 'en';
}

function t(key) {
  const lang = getLang();
  return TRANSLATIONS[lang]?.[key] ?? TRANSLATIONS['en'][key] ?? key;
}

function applyLang(lang) {
  if (!TRANSLATIONS[lang]) lang = 'en';
  localStorage.setItem('amr-lang', lang);
  document.documentElement.lang = lang;

  // 텍스트 노드 교체
  document.querySelectorAll('[data-i18n]').forEach(el => {
    const key = el.dataset.i18n;
    el.textContent = TRANSLATIONS[lang][key] ?? TRANSLATIONS['en'][key] ?? key;
  });

  // 랭 버튼 active 상태 업데이트
  document.querySelectorAll('.lang-btn').forEach(btn => {
    btn.classList.toggle('active', btn.dataset.lang === lang);
  });
}

function setLang(lang) {
  applyLang(lang);
}

// DOM 로드 후 저장된 언어 적용
document.addEventListener('DOMContentLoaded', () => applyLang(getLang()));
